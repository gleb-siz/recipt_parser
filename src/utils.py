...
def load_ocr(path):
    ...
    ocr_output_df[["x1", "y1", "x2", "y2", "x3", "y3", "x4", "y4"]] = (
        ocr_output_df.apply(lambda x: pd.Series(x["raw"].split(",")[:8]), axis=1)
    )
    ocr_output_df["text"] = ocr_output_df.apply(
        lambda x: ",".join(x["raw"].split(",")[8:]).replace("\n", ""), axis=1
    )
    ...
    ocr_output_df["x_max"] = ocr_output_df.apply(
        lambda x: max(x["x1"], x["x2"], x["x3"], x["x4"]), axis=1
    )
    ocr_output_df["y_max"] = ocr_output_df.apply(
        lambda x: max(x["y1"], x["y2"], x["y3"], x["y4"]), axis=1
    )
    ocr_output_df["x_min"] = ocr_output_df.apply(
        lambda x: min(x["x1"], x["x2"], x["x3"], x["x4"]), axis=1
    )
    ocr_output_df["y_min"] = ocr_output_df.apply(
        lambda x: min(x["y1"], x["y2"], x["y3"], x["y4"]), axis=1
    )
...
def add_features(df):
    ...
    df["file_aspect_ratio"] = df.apply(
        lambda x: x["width"] / (x["heigh"] + 0.00001), axis=1
    )
    ...
    df["aspect_ratio"] = df.apply(
        lambda x: x["token_width"] / (x["token_heigh"] + 0.00001), axis=1
    )
    ...
    df["has_total_keyword_in_row"] = df.groupby(["file", "row"])[
        "has_total_keyword"
    ].transform("max")
    ...
    df = (
        df.sort_values(["file", "text", "x_center", "y_center", "row_dist_from_total"])
        .groupby(["file", "text", "x_center", "y_center"], as_index=False)
        .first()
    )
    df = df.drop("row_total", axis=1)

    df = df.sort_values(["file", "row"])
    df["has_total_below"] = df.groupby("file")["has_total_keyword"].transform(
        lambda x: x.iloc[::-1].cummax().iloc[::-1]
    )
    df["has_total_below"] = (
        df.groupby("file")["has_total_below"].shift(-1).fillna(False)
    )
...
def fix_total_label(df, threshold=0.015):
    ...
    totals = df.copy()
    total_keywords = total_keywords[total_keywords["has_total_keyword"]][
        ["file", "text", "has_total_keyword", "y_center", "y_max"]
    ]
    ...
    totals = totals.sort_values(
        ["file", "label", "y_min"], ascending=[True, True, False]
    )
    ...
    df.loc[lambda x: x["label"] == "total", "label"] = df.loc[
        lambda x: x["label"] == "total", "label_true"
    ]
    df["label"] = df["label"].apply(lambda x: 1 if x == "total" else 0)

    # ad-hoc fix for own data input
    df.loc[
        lambda x: (x["file"] == "5807493927290997517")
        & (x["text"] == '"4,58"')
        & (x["y_center"] >= 0.330),
        "label",
    ] = 1
